...
export interface DriveOptions {
  ...
  /**
   * Штучна затримка перед кожним батчем — негативний контроль `T033`.
   *
   * Вимір часу мусить уміти не тільки назвати число, а й показати, що воно
   * чутливе до того, що міряє, і що воно відносить затримку туди, де вона
   * справді сталася. Відома затримка, вставлена **в наш** бік циклу, — це
   * єдиний спосіб довести обидва твердження на живому кластері: звіряч мусить
   * побачити рівно її, і мусить віднести її до простою драйвера, а не до MPC.
   *
   * У прогоні, який хтось замовив, цього параметра не буває: він задається
   * лише командою `sc003 --stall-ms` і в жодному шляху покупця не з'являється.
   */
  stallMs?: number
...
export async function drive(options: DriveOptions): Promise<DriveResult> {
  ...
  while (run.status === 'running' && run.datasetCursor < run.datasets.length) {
    ...
    for (const batch of batches) {
      // Затримка стоїть тут, а не після згортки, навмисно: вона мусить лягти в
      // ділянку «callback → перший запис наступного батча», тобто саме туди, де
      // живе наш простій на опитуванні ланцюга.
      const stall = options.stallMs ?? 0
      if (stall > 0) await new Promise((wake) => setTimeout(wake, stall))
