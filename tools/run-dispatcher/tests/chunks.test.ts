...
describe('нарізка батча на транзакції', () => {
  ...
  it('один батч коштує 10 транзакцій запису', () => {
    // Число тримається тестом, бо на ньому стоїть уся оцінка `SC-003`:
    // 2500 згорток × 10 — це 25 000 транзакцій на стандартний датасет. Менший
    // батч не зменшив кількості байтів у ланцюгу, і не мав.
    expect(writesPerBatch()).toBe(10)
  })
})
