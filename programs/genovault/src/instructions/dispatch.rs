...
/// приймає рівно 4 записи. Стандартний датасет — 10 000 записів, тобто 2500
/// згорток, і між ними мусить щось лишатись живим. Тому прогін розкладається
/// на чотири речі, кожна зі своєю причиною існувати:
///
/// - **буфер** (`["batch", run]`) — 8 832 байти, рівно один батч у тій
///   розкладці, в якій його читає черга обчислень. Інлайн-аргументами батч усе
///   одно не передати: індекс `ArgumentRef` це `u8`, тобто не більше 256
///   шифротекстів, а батч разом із накопичувачем це 300. Черга бере вхід рівно
///   з двох джерел, і друге — зріз Solana-акаунта, який вузли читають самі.
...
///
/// 2500 згорток — це 2500 підписів, і ще по 10 записів у буфер на кожну:
/// конверт їде в ланцюг транзакціями по ~900 байтів. Вкладка браузера, яка
/// просить тридцять тисяч підписів, — не продукт. Тому покупець при замовленні
...
/// транзакцій у Solana не буває, — тож один батч це 10 викликів. Порядок між
...
pub fn queue_fold(
    ...
    // `live` і чотири фільтри. Зріз акаунта покриває 276 параметрів поспіль —
