...
/// Скільки записів приймає одна згортка.
///
/// Чотири, а не тридцять два, і це не оптимізація, а межа тулчейну: Arcium
/// відхиляє визначення обчислення, чия вага перевищує **5 000 000 000 ACU**
/// (`InvalidCUAmount`), а `frequencies_fold` важить `0,861 + 0,522 × BATCH`
/// мільярда. При 32 це 17,6 млрд, при 8 — 5,04 млрд, тобто не влазить і те, і
/// те. Виміряно двома збірками контуру й двійковим пошуком по стелі; рішення
/// 2026-09-14 — брати 4 (2,95 млрд, запас 41 %), щоб наступна правка
/// рецепта не впиралась у стелю знову.
///
/// Сумарна вартість датасету від цього числа не залежить — вона на запис.
/// Залежить кількість **послідовних** згорток: накопичувач пускає одне
/// обчислення за раз, і 10 000 записів це 2500 кругів до MPC. Чи вкладається це
/// в `SC-003`, міряє `T033`.
...
/// Скільки байтів займає батч у буферному акаунті: 4 × 69 × 32 = 8 832.
pub const BATCH_PAYLOAD_BYTES: usize = RECIPE_BATCH * RECORD_WORDS * WORD_BYTES;

/// Скільки шифротекстів везе накопичувач.
///
/// `Pack<Frequencies>` пакує 76 польових елементів (`included`,
/// `dataset_included`, `male`, `affected`, 8 вікових порогів і 64 суми алелів)
/// по шість у шифротекст — звідси 13. Число з `.idarc`, а не з арифметики на
/// полях: пакування — властивість компілятора, і рахувати його вручну означало
/// б повторювати чужу реалізацію.
///
/// # Чому 13, а не 24
///
/// Вивід MPC повертається **однією транзакцією**: вузол відмовляється
/// доставляти callback, що не вміщається («multi-tx-callbacks feature
/// disabled»), і замість результату надсилає відмову. Виміряно на живому
/// кластері (`T030`): callback розкриття це `597 + 32 × N` байтів проти стелі
/// 1232, тобто `N ≤ 19`; при 24 прогін падав у `failed` без жодного натяку в
/// ланцюгу. Тому з рецепта прибрано `allele_square_sum` — продуктове рішення
/// 2026-09-14. Ціна названа: покупець бачить середнє число копій алеля, але не
/// розподіл генотипів, бо без `Σg²` він не відновлюється.
pub const ACCUMULATOR_CIPHERTEXTS: usize = 13;
...
/// інструкцію (`MAX_PERMITTED_DATA_INCREASE`). При `RECIPE_BATCH = 4` буфер у
/// неї вміщається цілком, тож `grow_batch_buffer` не викликається жодного разу
/// — але лишається: інструкція, якої немає, зробила б зміну `RECIPE_BATCH`
/// назад до восьми правкою в трьох місцях замість однієї константи.
pub const MAX_PERMITTED_DATA_INCREASE: usize = 10 * 1024;
pub const BATCH_BUFFER_INITIAL_BYTES: usize = if BATCH_BUFFER_BYTES < MAX_PERMITTED_DATA_INCREASE {
    BATCH_BUFFER_BYTES
} else {
...
mod tests {
    ...
    fn mirrors_the_recipe_shape() {
        ...
        assert_eq!(BATCH_PAYLOAD_BYTES, 8_832);
        assert_eq!(RECIPE_BATCH * RECORD_WORDS, 276);
    ...
    fn the_buffer_grows_within_the_runtime_limit() {
        ...
        // При `RECIPE_BATCH = 4` буфер створюється одразу цілим, тож дорощувань
        // немає. Цикл лишається перевіркою на майбутнє: батч, який знову не
        // вміститься в одну інструкцію, має доростати кроками, що в неї
        // вкладаються, а не падати в мережі.
        assert_eq!(steps, 0, "буфер на 8 896 байтів створюється одразу цілим");
